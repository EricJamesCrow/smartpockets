...
export function ConnectedBanks() {
    ...
    const banks = useQuery(api.dashboard.queries.getConnectedBanks, isAuthenticated ? {} : "skip");
    const [expandedBanks, setExpandedBanks] = useState<Set<string>>(new Set());

    if (!banks) {
        return (
            <div className="border-primary bg-primary rounded-xl border p-5">
                <h3 className="text-primary mb-4 text-lg font-semibold">Connected Banks</h3>
                <div className="space-y-3">
                    {[1, 2].map((i) => (
                        <div key={i} className="bg-secondary h-16 animate-pulse rounded" />
                    ))}
                </div>
            </div>
        );
    ...
    return (
        <div className="border-primary bg-primary rounded-xl border p-5">
            <div className="mb-4 flex items-center justify-between">
                <h3 className="text-primary text-lg font-semibold">Connected Banks</h3>
                <Link href="/settings/institutions" className="text-tertiary hover:text-secondary">
                    <Settings01 className="size-5" />
                </Link>
            </div>

            {banks.length === 0 ? (
                <p className="text-tertiary text-sm">No banks connected</p>
            ) : (
                <div className="space-y-3">
                        ...
                        return (
                            <div key={bank.itemId}>
                                <button
                                    ...
                                    className="hover:bg-secondary flex w-full items-center justify-between rounded-lg px-2 py-2 text-left"
                                >
                                    <div className="flex items-center gap-3">
                                        <Building07 className="text-tertiary size-5" />
                                        <div>
                                            <p className="text-primary text-sm font-medium">{bank.institutionName}</p>
                                            <p className={cx("text-xs", needsAttention ? "text-warning-600" : "text-tertiary")}>
                                ...
                                {isExpanded && (
                                    <div className="border-border-secondary ml-8 mt-1 space-y-1 border-l pl-4">
                                        {bank.accounts.map((acc) => (
                                            <div key={acc.accountId} className="flex items-center justify-between py-1 text-sm">
                                                <span className="text-secondary">
                                                    {acc.name}
                                                    {acc.mask && <span className="text-tertiary ml-1">••••{acc.mask}</span>}
                                                </span>
                                                <span className="text-tertiary">{acc.balance != null ? formatCurrency(acc.balance) : "—"}</span>
                                            </div>
                                        ))}
                                    </div>
                                )}
                    ...
                    {hiddenCount > 0 && (
                        <Link href="/settings/institutions" className="text-tertiary hover:text-secondary block text-center text-sm">
