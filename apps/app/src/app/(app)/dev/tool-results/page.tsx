    ...
    return (
        <div className="max-w-screen-md p-8 mx-auto">
            ...
            <p className="mb-6 text-sm text-tertiary">
                Dev-only. Each link renders every fixture state for that component with a theme toggle. Gated behind <code>NODE_ENV !== "production"</code> or{" "}
                <code>NEXT_PUBLIC_DEV_TOOLS=1</code>.
            </p>
            <ul className="divide-secondary divide-y">
                {COMPONENTS.map((c) => (
                    <li key={c} className="py-2">
                        <Link href={`/dev/tool-results/${c}`} className="text-primary hover:underline">
                            {c}
                        </Link>
                    </li>
                ))}
