 ...
 */
export function calculateUtilization(currentBalance: number | null | undefined, creditLimit: number | null | undefined): number | null {
 ...
 */
export function getUtilizationLevel(utilization: number | null): UtilizationLevel | null {
...
export function getUtilizationColor(utilization: number | null): string {
    if (utilization === null || utilization < 30) return "text-utility-success-600";
 ...
 */
export function getPurchaseApr(aprs: Array<{ aprPercentage: number; aprType: string }> | undefined): number | null {
    if (!aprs || aprs.length === 0) return null;

    const purchaseApr = aprs.find((apr) => apr.aprType.toLowerCase().includes("purchase") || apr.aprType.toLowerCase() === "purchase_apr");
 ...
 */
export function getPaymentUrgency(daysUntilDue: number | null): PaymentUrgency | null {
 ...
 */
export function getPaymentUrgencyBadgeColor(urgency: PaymentUrgency | null): "success" | "warning" | "orange" | "error" | "gray" {
    switch (urgency) {
        case "safe":
            return "success";
        case "warning":
 ...
 */
export function toExtendedCreditCard(card: CreditCard, cardholderName: string): ExtendedCreditCardData {
...
export function toTransaction(plaidTx: PlaidTransactionItem, cardId: string): Transaction {
    ...
    const frequency: Transaction["recurringFrequency"] =
        isRecurring && plaidTx.frequency
            ? plaidTx.frequency === "WEEKLY"
                ? "Weekly"
                : plaidTx.frequency === "BIWEEKLY"
                  ? "Biweekly"
                  : plaidTx.frequency === "SEMI_MONTHLY"
                    ? "Twice Monthly"
                    : plaidTx.frequency === "MONTHLY"
                      ? "Monthly"
                      : plaidTx.frequency === "ANNUALLY"
                        ? "Annual"
                        : "Recurring"
            : undefined;
