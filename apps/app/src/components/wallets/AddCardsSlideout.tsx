 ...
 */
export function AddCardsSlideout({ walletId, walletName, isOpen, onClose }: AddCardsSlideoutProps) {
    const [selectedCards, setSelectedCards] = useState<Set<Id<"creditCards">>>(new Set());
    const [isAdding, setIsAdding] = useState(false);

    // Get cards not already in this wallet
    const availableCards = useQuery(api.wallets.cardQueries.listNotInWallet, isOpen ? { walletId } : "skip");
    ...
    const isAllSelected = availableCards && availableCards.length > 0 && selectedCards.size === availableCards.length;

    return (
        <SlideoutMenu isOpen={isOpen} onOpenChange={(open) => !open && handleClose()}>
            <SlideoutMenu.Header onClose={handleClose}>
                <div className="flex flex-col gap-1">
                    <h2 className="text-primary text-lg font-semibold">Add Cards</h2>
                    <p className="text-tertiary text-sm">Select cards to add to &quot;{walletName}&quot;</p>
                </div>
            </SlideoutMenu.Header>
            ...
            <SlideoutMenu.Content>
                {availableCards === undefined ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-12 text-center">
                        <p className="text-tertiary text-sm">All your cards are already in this wallet</p>
                    </div>
                ) : (
                    <div>
                        {/* Select All */}
                        <div className="border-secondary mb-4 flex items-center gap-3 border-b pb-3">
                            <Checkbox
                                isSelected={isAllSelected ?? false}
                                isIndeterminate={selectedCards.size > 0 && selectedCards.size < availableCards.length}
                                onChange={toggleAll}
                            />
                            <span className="text-primary text-sm font-medium">Select All ({availableCards.length} cards)</span>
                        </div>
                            ...
                            {availableCards.map((card) => (
                                <label
                                    ...
                                    className={cx(
                                        ...
                                        selectedCards.has(card._id) ? "border-brand-primary bg-utility-brand-50" : "border-secondary hover:border-tertiary",
                                    )}
                                >
                                    <Checkbox isSelected={selectedCards.has(card._id)} onChange={() => toggleCard(card._id)} />
                                    <div className="min-w-0 flex-1">
                                        <div className="text-primary truncate font-medium">{card.displayName}</div>
                                        <div className="text-tertiary flex items-center gap-2 text-xs">
                                            {card.brand && <span className="capitalize">{card.brand}</span>}
            ...
            <SlideoutMenu.Footer className="flex items-center justify-end gap-3">
                ...
                <Button color="primary" size="md" onClick={handleAddCards} isLoading={isAdding} isDisabled={selectedCards.size === 0}>
