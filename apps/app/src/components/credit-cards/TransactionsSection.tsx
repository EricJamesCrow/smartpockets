...
import { type DetailPanelTransaction, TransactionDetailPanel } from "@/components/transactions/TransactionDetailPanel";
import { type PlaidTransactionItem, type Transaction, parseLocalDate, toTransaction } from "@/types/credit-cards";
import { TransactionFilters, type TransactionFiltersState, defaultTransactionFilters } from "./TransactionFilters";
import { type TransactionSortColumn, TransactionTableHeader } from "./TransactionTableHeader";
...
export function TransactionsSection({ cardId, accountId, filterMode = "all" }: TransactionsSectionProps) {
    ...
    const txResult = useQuery(api.transactions.queries.getTransactionsAndStreamsByAccountId, accountId ? { accountId } : "skip");

    // Convert Plaid transactions to our format
    const allTransactions = useMemo(() => {
        if (!txResult?.items) return [];
        return txResult.items.map((tx) => toTransaction(tx as PlaidTransactionItem, cardId as string));
    }, [txResult, cardId]);
    ...
    const filteredTransactions = useMemo(() => {
        ...
        if (filters.searchQuery) {
            ...
            result = result.filter((txn) => txn.merchant.toLowerCase().includes(query) || txn.description?.toLowerCase().includes(query));
        }
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2, delay: 0.1 }}>
            <TableCard.Root size="sm">
                <TableCard.Header
                    title={filterMode === "subscriptions" ? "Recurring Subscriptions" : filterMode === "transactions" ? "Transactions" : "Recent Transactions"}
                    ...
                    <div className="animate-pulse">
                        ...
                        {[...Array(5)].map((_, i) => (
                            <div key={i} className="border-secondary flex items-center gap-4 border-b px-4 py-3">
                                ...
                                <div className="bg-tertiary/20 h-5 w-20 rounded-full" />
                                <div className="bg-tertiary/30 h-4 w-16 rounded" />
                                <div className="bg-tertiary/20 h-5 w-16 rounded-full" />
                                <div className="bg-tertiary/20 h-4 w-24 rounded" />
                            </div>
                        ))}
                    </div>
                ) : (
                    <Table aria-label="Transactions" sortDescriptor={sortDescriptor} onSortChange={handleSortChange} size="sm">
                        ...
                        >
                            {(transaction) => <TransactionTableRow key={transaction.id} transaction={transaction} onSelect={handleSelectTransaction} />}
                        </AriaTableBody>
                    </Table>
                )}

                {/* Pagination */}
                {totalPages > 1 && <PaginationCardMinimal page={currentPage} total={totalPages} align="center" onPageChange={setCurrentPage} />}
            </TableCard.Root>
