...
/**
 * Smoke test for Variant C — Champagne Leather (CROWDEV-447, parent
 * CROWDEV-420).
 *
 * Covers the 9 must-keep features from the variant spec when the
 * `/wallets` page is wired to `variants/champagne-leather/SortableWalletCard`,
 ...
 * leather chassis to a bare champagne gradient.
 *
 *   1. Card renders with `data-variant="champagne-leather"` attribute
 ...
 *   9. Empty wallet (0 cards) shows the dark-ink "Empty wallet" affordance
 ...
 *
 * Selector / a11y conventions match the Variant A smoke spec (sibling).
 */

const CONVEX_URL =
  process.env.NEXT_PUBLIC_CONVEX_URL ?? process.env.CONVEX_URL ?? "";

const WALLET_NAME_PREFIX = "ChampagneLeatherTest-";

test.describe("Wallets variant C — Champagne Leather (CROWDEV-447)", () => {
  ...
  test.beforeAll(() => {
    if (!CONVEX_URL) {
      test.skip(
        true,
        "[wallets champagne-leather] NEXT_PUBLIC_CONVEX_URL is not set. Configure apps/app/.env.local per apps/app/tests/README.md.",
      );
    }
    if (!process.env.E2E_CLERK_USER_USERNAME) {
      test.skip(
        true,
        "[wallets champagne-leather] E2E_CLERK_USER_USERNAME is not set. Provision a Clerk dev test user per apps/app/tests/README.md.",
      );
    }
    if (
      ...
      test.skip(
        true,
        "[wallets champagne-leather] Neither CLERK_SECRET_KEY nor E2E_CLERK_USER_PASSWORD is set.",
      );
    }
  });
  ...
  test.beforeEach(async ({ page }) => {
    ...
    // Two wallets:
    //   - Pinned — exercises the champagne foil dot + ribbon-suffix copy
  ...
  test("card renders with data-variant='champagne-leather' attribute", async ({
    page,
  }) => {
    ...
    await expect(pinnedCard).toHaveAttribute(
      "data-variant",
      "champagne-leather",
    );
  });
  ...
  test("extended-view toggle reveals receipt stats", async ({ page }) => {
    ...
    // After enabling extended view, the champagne-grain panel renders the
    // four stat labels.
  ...
  test("empty wallet shows 'Empty wallet' affordance in italic Fraunces", async ({
    page,
  }) => {
    await page.goto("/wallets");
    ...
    // Seeded wallets have zero cards → the empty-state branch renders the
    // dark-ink Fraunces "Empty wallet" copy directly on the champagne
    // chassis (vs A's debossed cream variant).
  ...
  test("leather.png texture asset is reachable", async ({ page }) => {
    ...
    // CI time instead of in a visual review later. Same asset as Variant A.
