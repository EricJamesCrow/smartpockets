...
/**
 * Smoke test for Variant E — Refined + Glass (CROWDEV-444, parent
 * CROWDEV-420).
 *
 * Covers all 9 must-keep features from the variant spec when the
 * `/wallets` page is wired to
 * `variants/refined-glass/SortableWalletCard`:
 *
 *   1. Card renders with `data-variant="refined-glass"` attribute
 ...
 *
 * Variant E shares ~80% of Variant B's chassis, including all DOM
 * selectors — the only visual diffs (champagne glass veil, mouse-tracked
 * spotlight, moss ribbon instead of champagne, ambient page-level
 * backdrop-blur layer) don't change the accessible-name surface. The
 * one diff the smoke test asserts is `data-variant="refined-glass"`.
 *
 * Selectors / a11y notes (matches conventions in
 * `wallets-variant-refined.spec.ts`):
 *
 *   - Dropdown trigger's accessible name is "Open menu" (the
 *     hardcoded `aria-label` on `DropdownDotsButton` in `@repo/ui`
 *     wins over the variant's `aria-label="Options"` because of JSX
 *     attribute-order precedence).
 *   - Menu items have `role="menuitemradio"` (NOT `menuitem`) because
 *     `Dropdown.Menu` sets `selectionMode="single"`.
 *   - The Details toggle is a `react-aria-components` `<Switch>` with
 *     no aria-label; we click the wrapping
 ...
 *
 * Seeding strategy mirrors `wallets-variant-refined.spec.ts`:
 ...
 *   - Two wallets per test: one pinned (`RefinedGlassTest-Pinned`) so
 *     the pin indicator + ribbon can be asserted, one unpinned
 *     (`RefinedGlassTest-Plain`) so the dropdown toggle still has both
 *     "Pin / Unpin" labels available.
 */

const CONVEX_URL =
  process.env.NEXT_PUBLIC_CONVEX_URL ?? process.env.CONVEX_URL ?? "";

const WALLET_NAME_PREFIX = "RefinedGlassTest-";

test.describe("Wallets variant E — Refined + Glass (CROWDEV-444)", () => {
  let createdWalletIds: Id<"wallets">[] = [];

  // Same env-var skip pattern as the baseline + B specs so the suite
  ...
  test.beforeAll(() => {
    if (!CONVEX_URL) {
      test.skip(
        true,
        "[wallets refined-glass] NEXT_PUBLIC_CONVEX_URL is not set. Configure apps/app/.env.local per apps/app/tests/README.md.",
      );
    }
    if (!process.env.E2E_CLERK_USER_USERNAME) {
      test.skip(
        true,
        "[wallets refined-glass] E2E_CLERK_USER_USERNAME is not set. Provision a Clerk dev test user per apps/app/tests/README.md.",
      );
    }
    if (
      ...
      test.skip(
        true,
        "[wallets refined-glass] Neither CLERK_SECRET_KEY nor E2E_CLERK_USER_PASSWORD is set.",
      );
    }
  });
  ...
  test.beforeEach(async ({ page }) => {
    ...
    // the signed-in Clerk identity. Without it `viewerX()` throws and
    // the seed mutations below fail with "Authentication required".
    ...
    // Seed two wallets:
    //   - `Pinned`  — pinned, exercises the pin indicator + moss ribbon
    ...
    // assertion is also exercised by the first test that opens the
    // page.
  ...
  test("card renders with data-variant='refined-glass' attribute", async ({
    page,
  }) => {
    ...
    await expect(pinnedCard).toHaveAttribute("data-variant", "refined-glass");
  });
  ...
  test("displays wallet name and card count", async ({ page }) => {
    ...
    // Freshly-seeded wallets have zero cards. Scope the text query to
    // the card so we don't grab a stray "0 cards" elsewhere on the
    // page.
  ...
  test("pin indicator shows for pinned wallets", async ({ page }) => {
    ...
    // Pin01 SVG is rendered immediately after the wallet's `<h3>`
    // heading (moss-tinted on Variant E). The icon is aria-hidden, so
    // we identify it via the `h3 + svg` CSS adjacent-sibling combinator
    // — same approach as the baseline + B specs.
  ...
  test("drag handle becomes visible on hover", async ({ page }) => {
    ...
    // The drag handle lives in `SortableWalletCard`, so it's a sibling
    // of the `[data-testid="wallet-card"]` div — walk up one level to
    // the wrapper, then grab the labelled handle.
    const sortableWrapper = pinnedCard.locator("xpath=..");
    const dragHandle = sortableWrapper.getByLabel("Drag to reorder");

    // Before hover: handle is `opacity-0` but has non-zero dimensions,
    // so Playwright's `toBeVisible()` (display/visibility/dimensions
    // only, ignores opacity) returns true. Lock the opacity-based fade
    // with a direct CSS assertion.
  ...
  test("inline rename saves on Enter", async ({ page }) => {
    ...
    // type="text">`. `hasText` reads visible text, not input `value`,
    // so filtering by `originalName` drops to zero matches once the
    // input appears. Locate the rename input page-wide instead: only
    // one wallet can be in rename mode at a time.
    ...
    // Convex mutation re-renders the card with the new name in the
    // `<h3>`.
    await expect(
      page
        .locator('[data-testid="wallet-card"]')
  ...
  test("empty wallet shows 'Add a card' affordance in the veiled slot", async ({
    page,
  }) => {
    await page.goto("/wallets");
    ...
    // inside the dashed champagne slot (now with a glass `backdropFilter:
    // blur(4px)` to match the variant material).
