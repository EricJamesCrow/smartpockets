...
export interface CreditCardProps {
    ...
    className?: string;
    width?: number;
    /** Card network brand - determines which logo to display */
    brand?: CardBrand;
}

/**
 * Get the appropriate brand icon based on card brand and whether colored or white variant is needed
 */
...
export const CreditCard = ({
    ...
    // If width is provided, use fixed sizing with scale calculation
    // If width is undefined, use responsive sizing with aspect-ratio
    ...
    // Responsive mode: no fixed width prop, use w-full and aspect-ratio
    if (!width) {
        return (
            <div
                style={{ aspectRatio }}
            ...
            >
                <div
                    className={cx(
                        "absolute inset-0 flex flex-col justify-between overflow-hidden rounded-2xl p-4",
                        styles[type].root
                    )}
                >
                    {/* Horizontal strip */}
                    ...
                    <div className="relative flex items-end justify-between gap-3">
                        ...
                        <div className={cx("flex h-8 w-11.5 shrink-0 items-center justify-center rounded", styles[type].cardTypeRoot)}>
                            {(() => {
                                const useColoredVariant = CARD_WITH_COLOR_LOGO.includes(type as (typeof CARD_WITH_COLOR_LOGO)[number]);
                                const BrandIcon = getBrandIcon(brand, useColoredVariant);
                                return <BrandIcon />;
                            })()}
                        </div>
                    </div>
    ...
    // Fixed width mode: use scale-based sizing (for detail view, etc.)
    return (
        <div
            style={{
