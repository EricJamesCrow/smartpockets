...
export const MessageItem = ({ msg, showUserLabel = true, ...props }: MessageItemProps) => {
    ...
    return (
        <li key={msg.id} {...props} className={cx("relative flex items-start gap-3", msg.user?.me ? "self-end pl-10" : "pr-8 lg:pr-10", props.className)}>
            {msg.user && !msg.user.me && <Avatar src={msg.user.avatarUrl} placeholder={msg.user.avatar} alt={msg.user.name!} size="md" status={msg.user.status} />}
            ...
            <article className="flex min-w-0 flex-1 flex-col gap-1.5">
                ...
                {msg.text ? (
                    <div
                        className={cx(
                            "group/msg relative rounded-2xl px-5 py-3 text-md ring-inset",
                            msg.user?.me 
                                ? "rounded-tr-none bg-bg-brand-solid text-text-white pr-4" 
                                : "rounded-tl-none bg-secondary text-primary ring-1 ring-secondary",
                            // Link styles
                ...
                {msg.typing && (
                    <div className="inline-flex w-fit items-center gap-1 rounded-2xl rounded-tl-none bg-secondary px-5 py-3 ring-1 ring-secondary ring-inset min-h-[42px]">
                        <span className="size-2 rounded-full bg-fg-quaternary animate-bounce [animation-delay:-0.3s]" />
                        <span className="size-2 rounded-full bg-fg-quaternary animate-bounce [animation-delay:-0.15s]" />
                        <span className="size-2 rounded-full bg-fg-quaternary animate-bounce" />
                    </div>
                )}
            </article>
        </li>
