...
const SelectValue = ({ isOpen, isFocused, isDisabled, size, placeholder, placeholderIcon, ref, triggerClassName }: SelectValueProps) => {
    return (
        <AriaButton
            ref={ref}
            className={cx(
...
const Select = ({ placeholder = "Select", placeholderIcon, size = "sm", children, items, label, hint, tooltip, className, triggerClassName, ...rest }: SelectProps & { triggerClassName?: string }) => {
    return (
            ...
            <AriaSelect {...rest} className={(state) => cx("flex flex-col gap-1.5", typeof className === "function" ? className(state) : className)}>
                {(state) => (
                    <>
                        ...
                        <SelectValue {...state} {...{ size, placeholder }} placeholderIcon={placeholderIcon} triggerClassName={triggerClassName} />
