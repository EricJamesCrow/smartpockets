...
export const computeInterestSavingBalance = query({
    ...
    async handler(ctx, { creditCardId }) {
        ...
        const totalPromoBalances = promos.reduce((sum, p) => sum + p.remainingBalance, 0);
        const totalPromoMinPayments = promos.reduce((sum, p) => sum + (p.monthlyMinimumPayment ?? 0), 0);
        const totalInstallmentBalances = installments.reduce((sum, p) => sum + p.remainingPrincipal, 0);
        const totalInstallmentPayments = installments.reduce((sum, p) => sum + p.monthlyPrincipal + p.monthlyFee, 0);

        const totalProtectedBalances = totalPromoBalances + totalInstallmentBalances;
        const totalProtectedPayments = totalPromoMinPayments + totalInstallmentPayments;

        const interestSavingBalance = currentBalance - totalProtectedBalances + totalProtectedPayments;
...
export const computeYtdFeesInterest = query({
    ...
    async handler(ctx, { creditCardId }) {
        ...
        const transactions = await ctx.runQuery(components.plaid.public.getTransactionsByAccount, { accountId: card.accountId });
