...
export const getTransactionsAndStreamsByAccountId = query({
    ...
    handler: async (ctx, args) => {
        ...
        const transactions = await Promise.all(rawTransactions.map((tx) => enrichTransactionWithMerchant(ctx, tx, merchantCache)));
        ...
        let combined = [...transactions.map((t) => ({ ...t, type: "transaction" as const })), ...transformedStreams];
        ...
        if (args.searchQuery) {
            ...
            combined = combined.filter((item) => item.name.toLowerCase().includes(query) || item.merchantName?.toLowerCase().includes(query));
        }

        if (args.category && args.category !== "all") {
            combined = combined.filter((item) => item.categoryDetailed === args.category || item.categoryPrimary === args.category);
        }
...
export const listAllForUser = query({
    ...
    handler: async (ctx, args) => {
        ...
        type PlaidTransactionWithEnrichment = Awaited<ReturnType<typeof ctx.runQuery<typeof components.plaid.public.getTransactionsByAccount>>>[number] & {
        ...
        for (const accountId of allAccountIds) {
            ...
            const enrichedTransactions = await Promise.all(rawTransactions.map((tx) => enrichTransactionWithMerchant(ctx, tx, merchantCache)));
        ...
        // Category filter
        if (args.category && args.category !== "all") {
            transactions = transactions.filter((tx) => tx.categoryDetailed === args.category || tx.categoryPrimary === args.category);
        }
        ...
        // Amount filter args are display dollars; stored Plaid amounts are milliunits.
        ...
        if (args.amountMin !== undefined || args.amountMax !== undefined) {
            const amountMin = args.amountMin === undefined ? undefined : dollarsToMilliunits(args.amountMin);
            const amountMax = args.amountMax === undefined ? undefined : dollarsToMilliunits(args.amountMax);
