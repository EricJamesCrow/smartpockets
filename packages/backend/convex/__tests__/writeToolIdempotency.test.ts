...
import { createProposal, encodeReversalToken, executeWriteTool, registerToolExecutor, unregisterToolExecutor } from "../agent/writeTool";
...
function makeCtx(opts: { viewerId: string; proposal: FakeProposal; auditsByProposal: Map<string, FakeAudit[]> }) {
    return {
...
describe("executeWriteTool idempotency", () => {
    ...
    it("returns the original reversal token on retry when state is executed", async () => {
        ...
        const audits = new Map<string, FakeAudit[]>([[proposalId, [{ _id: auditId, executedAt: 1_000_000 }]]]);
...
describe("createProposal dedupe identity", () => {
    it("does not dedupe different patches against the same target", async () => {
        const viewerId = "user_abc";
        const threadId = "thread_123";
        const inserted: Array<{ _id: string; contentHash: string }> = [];
        const ctx = {
            viewerX: () => ({ _id: viewerId }),
            table: (name: string) => {
                if (name === "agentThreads") {
                    return {
                        getX: async () => ({ userId: viewerId, readCallCount: 1 }),
                    };
                }
                if (name === "agentProposals") {
                    return {
                        insert: async (row: { contentHash: string }) => {
                            if (inserted.some((existing) => existing.contentHash === row.contentHash)) {
                                throw new Error("contentHash unique constraint");
                            }
                            const _id = `proposal_${inserted.length + 1}`;
                            inserted.push({ ...row, _id });
                            return _id;
                        },
                        get: async (_field: string, contentHash: string) => inserted.find((row) => row.contentHash === contentHash) ?? null,
                    };
                }
                throw new Error(`unexpected table ${name}`);
            },
        ...
        const base = {
            toolName: "propose_transaction_update",
            summaryText: "Update tx",
            affectedCount: 1,
            affectedIds: ["tx_1"],
            sampleJson: "{}",
            scope: "single" as const,
            threadId: threadId as any,
            awaitingExpiresAt: Date.now() + 60_000,
        };

        const first = await createProposal(ctx as any, {
            ...base,
            argsJson: JSON.stringify({ plaidTransactionId: "tx_1", patch: { userCategory: "Food" } }),
        });
        const second = await createProposal(ctx as any, {
            ...base,
            argsJson: JSON.stringify({ plaidTransactionId: "tx_1", patch: { userCategory: "Travel" } }),
        });
        ...
        expect(inserted[0]?.contentHash).not.toBe(inserted[1]?.contentHash);
    });
});
