...
import { buildPromoAffectedIds } from "../agent/tools/propose/proposeManualPromo";
import {
  getReversalHandler,
  getToolExecutor,
...
function makeCtx(opts: {
  ...
  const promos = new Map<string, Promo>(
    opts.promos.map((p) => [p._id, { ...p }]),
  );
  let nextId = 1;

  const cardHandle = (id: string) => ({
    _id: id,
    get userId() { return cards.get(id)!.userId; },
    get displayName() { return cards.get(id)!.displayName; },
  });
  ...
  const promoHandle = (id: string) => ({
    ...
    get originalBalance() { return promos.get(id)!.originalBalance; },
    get remainingBalance() { return promos.get(id)!.remainingBalance; },
    get startDate() { return promos.get(id)!.startDate; },
    get expirationDate() { return promos.get(id)!.expirationDate; },
    get isDeferredInterest() { return promos.get(id)!.isDeferredInterest; },
    get isActive() { return promos.get(id)!.isActive; },
...
describe("propose_manual_promo executor", () => {
  it("uses promo-specific affected ids for create dedupe", () => {
    expect(buildPromoAffectedIds("card_1", basePromo)).toEqual([
      "new:card_1:2026-01-01:2026-12-31:purchase apr 0%",
    ]);
    expect(
    ...
    expect(
      buildPromoAffectedIds("card_1", { ...basePromo, promoRateId: "promo_1" }),
    ).toEqual(["promo_1"]);
  });
