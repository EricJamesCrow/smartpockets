...
export const compose = internalQuery({
    ...
    handler: async (ctx, { userId, threadId }) => {
        ...
        const [accounts, cards, openProposals] = await Promise.all([
            ctx.runQuery(internal.plaidComponent.getAccountsByTrustedUserId, {
                userId: user.externalId,
            }) as Promise<unknown[]>,
            ctx.table("creditCards", "by_user_active", (q) => q.eq("userId", userId).eq("isActive", true)),
            ctx.runQuery((internal as any).agent.proposals.countOpenForThreadInternal, { threadId }) as Promise<number>,
        ]);
