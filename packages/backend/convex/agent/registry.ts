...
export const AGENT_TOOLS: Record<string, ToolDef> = {
  ...
  list_accounts: {
    description:
      "List the user's bank and credit card accounts, optionally filtered by type.",
  ...
  get_account_detail: {
    description:
      "Get a single account's full detail including balances and metadata.",
    llmInputSchema: z.object({
  ...
  list_transactions: {
    description:
      "List transactions, optionally filtered by account, date range, and limit.",
    llmInputSchema: z.object({
  ...
  get_transaction_detail: {
    description:
      "Get a single transaction's full detail including any user overlay.",
    llmInputSchema: z.object({
  ...
  get_credit_card_detail: {
    description:
      "Get a credit card's full detail including APRs, balances, ISB, and YTD fees.",
    llmInputSchema: z.object({
  ...
  list_deferred_interest_promos: {
    description:
      "List active deferred-interest promos and their expiration dates.",
    llmInputSchema: z.object({
  ...
  get_spend_by_category: {
    description:
      "Aggregate spending by category over a date range.",
    llmInputSchema: z.object({
      dateFrom: z.string(),
      dateTo: z.string(),
  ...
  get_upcoming_statements: {
    description:
      "List credit cards with a statement closing in the next N days.",
    llmInputSchema: z.object({
  ...
  search_merchants: {
    description:
      "Fuzzy search for merchants by name. Returns top matches by transaction count.",
    llmInputSchema: z.object({
  ...
  get_plaid_health: {
    description:
      "Health summary for the user's Plaid items (stub until W4 publishes).",
    llmInputSchema: z.object({}),
  ...
  propose_transaction_update: {
    description:
      "Propose an update to a single transaction's overlay. User confirms before execute.",
    llmInputSchema: z.object({
      plaidTransactionId: z.string(),
      overlay: z.object({
  ...
  propose_bulk_transaction_update: {
    description:
      "Propose a bulk update to a filtered set of transactions. Returns a proposal the user must confirm before execute.",
    llmInputSchema: z.object({
      filter: z
        .object({
  ...
  propose_manual_promo: {
    description:
      "Propose adding a manual (user-entered) deferred-interest promo for a card.",
    llmInputSchema: z.object({
      cardId: z.string(),
      promo: z.object({
    ...
    llmInputSchema: z.object({
      reminder: z.object({
        ...
        relatedResourceType: z
          .enum(["creditCard", "promoRate", "installmentPlan", "transaction", "none"])
          .default("none"),
  ...
  cancel_proposal: {
    description:
      "Cancel a proposal that is awaiting confirmation. Idempotent.",
    llmInputSchema: z.object({
      proposalId: z.string(),
    }),
  ...
  undo_mutation: {
    description:
      "Reverse a recently executed proposal within its undo window.",
    llmInputSchema: z.object({
  ...
  trigger_plaid_resync: {
    description:
      "Manually trigger a Plaid resync for one or all of the user's linked items.",
    llmInputSchema: z.object({
      plaidItemId: z.string().optional(),
      scope: z
        .enum(["accounts", "transactions", "liabilities", "all"])
        .optional(),
    }),
  ...
  get_proposal: {
    description:
      "Inspect a proposal's current state (reconciliation M11; W3 subscribes via useQuery).",
    llmInputSchema: z.object({
      proposalId: z.string(),
    }),
