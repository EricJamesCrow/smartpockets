...
export const list = query({
    ...
    async handler(ctx) {
        ...
        const wallets = await ctx.table("wallets", "by_user_sortOrder", (q) => q.eq("userId", viewer._id));
...
export const listPinned = query({
    ...
    async handler(ctx) {
        ...
        const allWallets = await ctx.table("wallets", "by_user_pinned", (q) => q.eq("userId", viewer._id).eq("isPinned", true));

        // Sort by pinnedSortOrder in memory (index doesn't cover this)
        const wallets = [...allWallets].sort((a, b) => a.pinnedSortOrder - b.pinnedSortOrder);

        // Map wallets with all cards
        const walletsWithCards = await Promise.all(
            wallets.map(async (wallet) => {
                // Get wallet cards sorted by sortOrder
                const walletCards = await ctx.table("walletCards", "by_wallet_sortOrder", (q) => q.eq("walletId", wallet._id));
...
export const get = query({
    ...
    async handler(ctx, { walletId }) {
        ...
        const averageUtilization = totalCreditLimit > 0 ? Math.round((totalBalance / totalCreditLimit) * 100 * 100) / 100 : 0;
