...
export const listByWallet = query({
    ...
    async handler(ctx, { walletId }) {
        ...
        const walletCards = await ctx.table("walletCards", "by_wallet_sortOrder", (q) => q.eq("walletId", walletId));
        ...
        return cards
            .filter(
                (card): card is NonNullable<typeof card> =>
                    card !== null && card.userId === viewer._id && card.isActive && (!card.plaidItemId || activeItemIds.has(card.plaidItemId)),
            )
            .map((card) => card.doc());
    },
});
...
export const listNotInWallet = query({
    ...
    async handler(ctx, { walletId }) {
        ...
        const activeCards = allCards.filter((card) => !card.plaidItemId || activeItemIds.has(card.plaidItemId));
