...
export const getHeroMetrics = query({
    ...
    async handler(ctx) {
        ...
        const utilizationPercent = totalCreditLimit > 0 ? (totalBalance / totalCreditLimit) * 100 : 0;
    ...
    async handler(ctx) {
        ...
        const payments = cards
            ...
            .map((card) => {
                ...
                const isPaid =
                    card.lastPaymentDate && card.lastStatementIssueDate ? new Date(card.lastPaymentDate) > new Date(card.lastStatementIssueDate) : false;
...
export const getConnectedBanks = query({
    ...
    async handler(ctx) {
        ...
        const healths = await ctx.runQuery(components.plaid.public.getItemHealthByUser, { userId: viewer.externalId });

        const banks = await Promise.all(
            healths.map(async (h: any) => {
                const accounts = await ctx.runQuery(components.plaid.public.getAccountsByItem, { plaidItemId: h.plaidItemId });

                // Derive legacy `status` from the rich state for back-compat.
                const legacyStatus =
                    h.state === "re-consent-required" ? "needs_reauth" : h.state === "error" ? "error" : h.state === "syncing" ? "syncing" : "active";
...
export const getSpendingBreakdown = query({
    ...
    async handler(ctx, { period = "this_month" }) {
        ...
        for (const itemId of activeItemIds) {
            const accounts = await ctx.runQuery(components.plaid.public.getAccountsByItem, { plaidItemId: itemId });

            for (const acc of accounts) {
                const txs = await ctx.runQuery(components.plaid.public.getTransactionsByAccount, { accountId: acc.accountId });
        ...
        // Group by category. Transaction amounts stay in canonical milliunits.
...
export const getRecentTransactions = query({
    ...
    async handler(ctx, { limit = 10 }) {
        ...
        const accountToCard = new Map(allCards.map((card) => [card.accountId, { name: card.displayName, lastFour: card.lastFour }]));
        ...
        return allTransactions.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).slice(0, limit);
    },
});
