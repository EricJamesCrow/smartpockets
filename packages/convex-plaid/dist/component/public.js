...
// =============================================================================
const propertyAddressValidator = v.optional(v.object({
...
// =============================================================================
const servicerAddressValidator = v.optional(v.object({
    street: v.optional(v.string()),
    city: v.optional(v.string()),
    region: v.optional(v.string()),
    postalCode: v.optional(v.string()),
...
const loanStatusValidator = v.optional(v.object({
    type: v.optional(v.string()),
    endDate: v.optional(v.string()),
}));
const repaymentPlanValidator = v.optional(v.object({
    type: v.optional(v.string()),
...
// SYNC LOGS QUERIES
// =============================================================================
const syncResultValidator = v.optional(v.object({
...
export const getSyncStats = query({
    ...
    handler: async (ctx, args) => {
        ...
        const averageDurationMs = completedLogs.length > 0
            ? Math.round(completedLogs.reduce((sum, log) => sum + (log.durationMs ?? 0), 0) /
                completedLogs.length)
            : undefined;
        // Find last sync times
        const sortedByStart = [...recentLogs].sort((a, b) => b.startedAt - a.startedAt);
        const lastSync = sortedByStart[0];
        const lastSuccess = successLogs.sort((a, b) => b.startedAt - a.startedAt)[0];
...
// W4: ITEM HEALTH
// =============================================================================
import { derive } from "./health.js";
const reasonCodeValidator = v.union(v.literal("healthy"), v.literal("syncing_initial"), v.literal("syncing_incremental"), v.literal("auth_required_login"), v.literal("auth_required_expiration"), v.literal("transient_circuit_open"), v.literal("transient_institution_down"), v.literal("transient_rate_limited"), v.literal("permanent_invalid_token"), v.literal("permanent_item_not_found"), v.literal("permanent_no_accounts"), v.literal("permanent_access_not_granted"), v.literal("permanent_products_not_supported"), v.literal("permanent_institution_unsupported"), v.literal("permanent_revoked"), v.literal("permanent_unknown"), v.literal("new_accounts_available"));
const itemHealthValidator = v.object({
    plaidItemId: v.string(),
    itemId: v.string(),
    state: v.union(v.literal("syncing"), v.literal("ready"), v.literal("error"), v.literal("re-consent-required")),
    recommendedAction: v.union(v.literal("reconnect"), v.literal("reconnect_for_new_accounts"), v.literal("wait"), v.literal("contact_support"), v.null()),
...
async function getInstitutionSnapshot(ctx, institutionId) {
    if (!institutionId) {
        return {
            institutionId: null,
...
async function getLastWebhookAt(ctx, itemId) {
...
const webhookStatusValidator = v.union(v.literal("received"), v.literal("processing"), v.literal("processed"), v.literal("duplicate"), v.literal("failed"));
function isWebhookDedupeCandidate(status) {
...
export const recordWebhookReceived = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const recent = await ctx.db
            .query("webhookLogs")
            .withIndex("by_body_hash_received_at", (q) => q.eq("bodyHash", args.bodyHash).gte("receivedAt", cutoff))
            .order("desc")
            .take(10);
        const duplicateOf = recent.find((log) => isWebhookDedupeCandidate(log.status));
...
export const getItemHealth = query({
    ...
    handler: async (ctx, args) => {
        ...
        return derive({ ...item, _id: String(item._id) }, institution, lastWebhookAt);
    },
});
/**
...
export const getItemHealthByUser = query({
    ...
    handler: async (ctx, args) => {
        ...
        return await Promise.all(visible.map(async (item) => {
            const institution = await getInstitutionSnapshot(ctx, item.institutionId);
            const lastWebhookAt = await getLastWebhookAt(ctx, item.itemId);
            return derive({ ...item, _id: String(item._id) }, institution, lastWebhookAt);
        }));
