 ...
 */
async function verifyPlaidWebhookInternal(jwt, rawBody, plaidConfig, forceRefresh = false) {
 ...
 */
export async function verifyPlaidWebhook(jwt, rawBody, plaidConfig) {
    try {
        // First attempt: try with cached key (if available)
        const result = await verifyPlaidWebhookInternal(jwt, rawBody, plaidConfig, false);
        if (result.isValid) {
            return { isValid: true };
        }
        ...
        if (kid && keyCache.has(kid)) {
            ...
            try {
                const retryResult = await verifyPlaidWebhookInternal(jwt, rawBody, plaidConfig, true // Force refresh
                );
 ...
 */
export function invalidateCachedKey(kid) {
    keyCache.delete(kid);
}
/**
 ...
 */
export function isKeyCached(kid) {
 ...
 */
async function getVerificationKey(kid, plaidConfig, forceRefresh = false) {
    ...
    const configuration = new Configuration({
        basePath: PlaidEnvironments[plaidConfig.plaidEnv] || PlaidEnvironments.sandbox,
        baseOptions: {
            headers: {
 ...
 */
export function isInitialUpdateWebhook(webhookType, webhookCode) {
    return webhookType === "TRANSACTIONS" && webhookCode === "INITIAL_UPDATE";
}
/**
 * Check if this is a historical transactions update.
 */
export function isHistoricalUpdateWebhook(webhookType, webhookCode) {
 ...
 */
export function generateWebhookId(itemId, webhookCode, timestamp) {
