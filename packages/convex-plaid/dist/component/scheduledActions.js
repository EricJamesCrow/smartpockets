...
export const syncAllActiveItems = internalAction({
    args: {
        ...
        syncType: v.optional(v.union(v.literal("transactions"), v.literal("liabilities"), v.literal("recurring"), v.literal("all"))),
    },
    returns: v.object({
        scheduled: v.number(),
    }),
...
export const syncSingleItem = internalAction({
    args: {
        ...
        plaidConfig: plaidConfigValidator,
        syncType: v.union(v.literal("transactions"), v.literal("liabilities"), v.literal("recurring"), v.literal("all")),
    },
    returns: v.object({
        success: v.boolean(),
    ...
    handler: async (ctx, args) => {
        try {
            // Get item details
            const item = await ctx.runQuery(internal.private.getItemWithToken, {
            ...
            const accessToken = await decryptToken(item.accessToken, args.plaidConfig.encryptionKey);
            const plaidClient = initPlaidClient(args.plaidConfig.plaidClientId, args.plaidConfig.plaidSecret, args.plaidConfig.plaidEnv);
            // Sync transactions
            ...
            if (args.syncType === "liabilities" || args.syncType === "all") {
                await syncItemLiabilities(ctx, item._id, args.plaidConfig.plaidClientId, args.plaidConfig.plaidSecret, args.plaidConfig.plaidEnv, args.plaidConfig.encryptionKey);
            }
            // Sync recurring streams
            if (args.syncType === "recurring" || args.syncType === "all") {
                await syncItemRecurringStreams(ctx, item._id, args.plaidConfig.plaidClientId, args.plaidConfig.plaidSecret, args.plaidConfig.plaidEnv, args.plaidConfig.encryptionKey);
            }
            ...
            console.error(`[Plaid Cron] Error syncing item ${args.plaidItemId}: ${formatErrorForLog(plaidError)}`);
...
export const syncStaleItems = internalAction({
    ...
    handler: async (ctx, args) => {
        ...
        console.log(`[Plaid Cron] Scheduling sync for items not updated in ${maxAgeHours} hours...`);
 ...
 */
async function syncItemTransactions(ctx, item, accessToken, plaidClient) {
    // Mark as syncing
    await ctx.runMutation(internal.private.updateItemStatus, {
        plaidItemId: item._id,
        status: "syncing",
    ...
    const syncResult = await syncTransactionsPaginated(plaidClient, accessToken, item.cursor ?? "");
 ...
 */
async function syncItemLiabilities(_ctx, plaidItemId, _plaidClientId, _plaidSecret, _plaidEnv, _encryptionKey) {
 ...
 * This is because actions cannot call other actions in Convex components.
 */
async function syncItemRecurringStreams(_ctx, plaidItemId, _plaidClientId, _plaidSecret, _plaidEnv, _encryptionKey) {
    // Note: In Convex components, actions cannot call other actions.
