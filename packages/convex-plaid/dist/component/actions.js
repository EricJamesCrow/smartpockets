...
import { initPlaidClient, convertAmountToMilliunits, transformTransaction, syncTransactionsPaginated, normalizePlaidProducts, selectMerchantCounterparty, partitionEnrichmentInput, } from "./utils.js";
...
export const fetchAccounts = action({
    ...
    handler: async (ctx, args) => {
        ...
        try {
            ...
            console.error(`[Plaid Component] Accounts error: ${formatErrorForLog(plaidError)}`);
...
export const syncTransactions = action({
    ...
    handler: async (ctx, args) => {
        ...
        if (!lockResult.acquired) {
            console.log(`[Plaid Component] Sync skipped: ${lockResult.reason}`);
            return {
                added: 0,
                modified: 0,
                removed: 0,
        ...
        console.log(`[Plaid Component] Lock acquired (version ${syncVersion}), cursor: ${initialCursor?.substring(0, 20) || "empty"}...`);
        // Create sync log entry
        const syncLogId = await ctx.runMutation(internal.private.createSyncLog, {
            plaidItemId: args.plaidItemId,
            userId,
        ...
        try {
            ...
            const syncResult = await syncTransactionsPaginated(plaidClient, accessToken, initialCursor ?? "", {
                maxPages: args.maxPages,
                maxTransactions: args.maxTransactions,
            });
            console.log(`[Plaid Component] Sync fetched: ${syncResult.added.length} added, ` +
            ...
            const completeResult = await ctx.runMutation(internal.private.completeSyncWithVersion, {
                plaidItemId: args.plaidItemId,
                syncVersion,
            ...
            if (!completeResult.success) {
                ...
                console.warn(`[Plaid Component] Sync complete failed: ${completeResult.reason}`);
            ...
            const plaidError = categorizeError(error);
            console.error(`[Plaid Component] Sync error: ${formatErrorForLog(plaidError)}`);
            // Complete sync log with error
            await ctx.runMutation(internal.private.completeSyncLogError, {
                syncLogId,
                errorCode: plaidError.code,
...
export const backfillTransactionEnrichments = action({
    ...
    handler: async (ctx, args) => {
        console.log(`[Plaid Component] Backfilling transaction enrichments for ${args.plaidItemId}...`);
        const item = await ctx.runQuery(internal.private.getPlaidItem, {
            plaidItemId: args.plaidItemId,
        });
        if (!item) {
        ...
        const transactions = [...syncResult.added, ...syncResult.modified].map((transaction) => transformTransaction(transaction));
        const backfillResult = await ctx.runMutation(internal.private.backfillTransactionEnrichments, {
            plaidItemId: args.plaidItemId,
            transactions,
        });
        return {
...
export const fetchLiabilities = action({
    ...
    handler: async (ctx, args) => {
        ...
        try {
            ...
            console.log(`[Plaid Component] Fetched ${creditCards.length} credit cards, ${mortgages.length} mortgages, ${studentLoans.length} student loans`);
            ...
            if (studentLoans.length > 0) {
                await ctx.runMutation(internal.private.bulkUpsertStudentLoanLiabilities, {
                    ...
                    studentLoans: studentLoans.map((loan) => ({
                        ...
                        minimumPaymentAmount: loan.minimum_payment_amount != null
            ...
            console.log(`[Plaid Component] Stored ${creditCards.length} credit cards, ${mortgages.length} mortgages, ${studentLoans.length} student loans`);
            // Complete sync log with success
            await ctx.runMutation(internal.private.completeSyncLogSuccess, {
                syncLogId,
                result: {
            ...
            const plaidError = categorizeError(error);
            console.error(`[Plaid Component] Liabilities error: ${formatErrorForLog(plaidError)}`);
            // Complete sync log with error
            await ctx.runMutation(internal.private.completeSyncLogError, {
                syncLogId,
                errorCode: plaidError.code,
...
export const fetchRecurringStreams = action({
    ...
    handler: async (ctx, args) => {
        ...
        try {
            ...
            console.log(`[Plaid Component] Fetched ${inflowStreams.length} inflows, ${outflowStreams.length} outflows`);
            ...
            const plaidError = categorizeError(error);
            console.error(`[Plaid Component] Recurring streams error: ${formatErrorForLog(plaidError)}`);
            // Complete sync log with error
            await ctx.runMutation(internal.private.completeSyncLogError, {
                syncLogId,
                errorCode: plaidError.code,
...
export const enrichTransactions = action({
    ...
    handler: async (ctx, args) => {
        console.log(`[Plaid Component] Enriching ${args.transactions.length} transactions...`);
        ...
        if (partitioned.skipped.length > 0) {
            console.log(`[Plaid Component] Skipping ${partitioned.skipped.length} transaction(s) with unsupported account_type.`);
        }
        let totalEnriched = 0;
        let totalFailed = 0;
        for (const [accountType, batch] of [
            ...
            try {
                const response = await plaidClient.transactionsEnrich({
                    ...
                    transactions: enrichmentTransactions,
                });
                for (const enrichedTx of response.data.enriched_transactions) {
                    ...
                    if (counterparty?.entity_id && counterparty.name) {
                        await ctx.runMutation(internal.private.upsertMerchantEnrichment, {
                            ...
                            confidenceLevel: counterparty.confidence_level ?? "UNKNOWN",
                        });
                ...
                console.error(`[Plaid Component] Transaction enrichment failed (account_type=${accountType}):`, formatErrorForLog(error));
                throw error;
            }
        }
        totalFailed += partitioned.skipped.length;
        console.log(`[Plaid Component] Enriched ${totalEnriched} transactions, ${totalFailed} failed (incl. ${partitioned.skipped.length} skipped).`);
...
export const triggerTransactionsRefresh = action({
    ...
    handler: async (ctx, args) => {
        console.log("[Plaid Component] Triggering transactions refresh for item:", args.plaidItemId);
        ...
        try {
            ...
            console.log("[Plaid Component] Transactions refresh triggered successfully:", response.data.request_id);
            return {
                success: true,
            ...
            if (errorCode === "PRODUCTS_NOT_SUPPORTED" ||
                ...
                console.warn("[Plaid Component] Transactions refresh not supported for this institution");
                return {
                    success: false,
            ...
            console.error("[Plaid Component] Transactions refresh failed:", formatErrorForLog(error));
            return {
                success: false,
