 ...
 * ```
 */
export async function requireOwnership(ctx, resourceUserId) {
    const identity = await ctx.auth.getUserIdentity();
    if (!identity?.subject) {
        throw new Error("Authentication required");
    }
 ...
 */
export async function requireItemOwnership(ctx, plaidItemId, plaidApi) {
    const userId = await requireAuth(ctx);
 ...
 */
export async function requireAccountOwnership(ctx, accountId, plaidApi) {
    const userId = await requireAuth(ctx);
