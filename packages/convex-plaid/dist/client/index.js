...
import { verifyPlaidWebhook, parseWebhookPayload, isTransactionSyncWebhook, isItemErrorWebhook, isPendingExpirationWebhook, isUserPermissionRevokedWebhook, isLiabilitiesUpdateWebhook, } from "../component/webhooks.js";
// =============================================================================
// CONFIG VALIDATION
// =============================================================================
/**
 ...
 */
const VALID_PLAID_ENVS = ["sandbox", "development", "production"];
/**
...
function validatePlaidConfig(config) {
    ...
    for (const field of requiredFields) {
        ...
        if (typeof value !== "string" || value.trim() === "") {
            throw new PlaidConfigError(`${field} is required and must be a non-empty string`);
        }
    }
    // Validate PLAID_ENV
    if (!VALID_PLAID_ENVS.includes(config.PLAID_ENV)) {
        throw new PlaidConfigError(`PLAID_ENV must be one of: ${VALID_PLAID_ENVS.join(", ")}. Got: "${config.PLAID_ENV}"`);
    }
    ...
    try {
        ...
        if (decoded.length !== 32) {
            throw new PlaidConfigError(`ENCRYPTION_KEY must be a base64-encoded 256-bit (32 byte) key. Got ${decoded.length} bytes.`);
        }
    }
    catch (e) {
...
export class Plaid {
     ...
     */
    async exchangePublicToken(ctx, args) {
     ...
     */
    async syncTransactions(ctx, args) {
     ...
     * @param plaidItemId - Convex document ID of the plaidItem (as string)
     */
    async fetchLiabilities(ctx, args) {
     ...
     * @param plaidItemId - Convex document ID of the plaidItem (as string)
     */
    async triggerTransactionsRefresh(ctx, args) {
     ...
     */
    async enrichTransactions(ctx, args) {
        // Cast to any since generated types may not include this action yet
        const actions = this.component.actions;
     ...
     * @param plaidItemId - Convex document ID of the plaidItem (as string)
     */
    async fetchRecurringStreams(ctx, args) {
        ...
        handler: httpActionGeneric(async (ctx, req) => {
            ...
            if (config?.plaidConfig && signedJwt) {
                ...
                if (!verification.isValid) {
                    console.error(`[Plaid Webhook] Signature verification failed: ${verification.error}`);
            ...
            const componentInternal = component;
            // Step 3: Look up the plaidItem by Plaid's item_id
            const plaidItem = await ctx.runQuery(componentInternal.private.getPlaidItemByItemId, { itemId: item_id });
            if (!plaidItem) {
                console.warn(`[Plaid Webhook] No plaidItem found for item_id: ${item_id}`);
                // Still return 200 to prevent Plaid from retrying
                return new Response(JSON.stringify({ received: true, warning: "Item not found" }), { status: 200, headers: { "Content-Type": "application/json" } });
            }
            const plaidItemId = String(plaidItem._id);
            // Step 4: Handle webhook by type
            if (isTransactionSyncWebhook(webhook_type, webhook_code)) {
                console.log(`[Plaid Webhook] Transaction updates available for item: ${item_id}`);
                ...
                console.error(`[Plaid Webhook] Item error for ${item_id}: ${errorCode} - ${errorMessage}`);
                ...
                console.log(`[Plaid Webhook] Item ${item_id} access token expiring soon`);
                ...
                console.log(`[Plaid Webhook] Liability updates available for item: ${item_id}`);
                ...
                console.log(`[Plaid Webhook] Unhandled webhook: ${webhook_type}.${webhook_code}`);
            }
            // Step 5: Call custom handler if provided
            if (config?.onWebhook) {
                try {
                    await config.onWebhook(ctx, webhook_type, webhook_code, item_id, payload);
                }
                catch (e) {
